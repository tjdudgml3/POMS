...
from poms import spectral_library
from poms import writer
from poms.config import config


def poms(spectral_library_filename: str, query_filename: str,
             out_filename: str, **kwargs: Union[bool, float, int, str]) -> int:
    """
    Run POMS with the specified search settings.
    ...
    **kwargs : Union[bool, float, int, str]
        ...
        https://github.com/bittremieux/POMS/wiki/Parameters). Values
    ...
    -------
    int
        The error code from running POMS.
    """
...
#     identifications = spec_lib.search(config.query_filename)
    ...
    with open(f"12_16_3_query_seq_emb_method_seq_HEK_cand.txt", "w") as f:
